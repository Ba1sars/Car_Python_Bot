...
async def inline_marque_buttons():
    ...
    for marque in marque_array:
        keyboard.add(
            InlineKeyboardButton(text=str(marque), callback_data=f"marque_{marque}")
        )
...
async def inline_model_buttons(state: FSMContext):
    ...
    for model in model_array:
        keyboard.add(
            InlineKeyboardButton(text=str(model), callback_data=f"model_{model}")
        )
...
async def inline_series_with_publish_year_buttons(state: FSMContext):
    ...
    series_array = set(
        fetch_factory(user_data["user_marque"], model, "series_with_publish_year")
    )
    for series in series_array:
        keyboard.add(
            InlineKeyboardButton(
                text=str(series), callback_data=f"series_year_{series}"
            )
        )
...
async def inline_first_photo_link_buttons(state: FSMContext):
    ...
    keyboard.add(
        InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_series_year")
    )
    keyboard.add(
        InlineKeyboardButton(text="Выбрать", callback_data=f"photo_link_{version}")
    )
...
async def inline_next_photo_link_buttons(state: FSMContext):
    ...
    version = f"Версия {user_data['current_index'] + 1}"
    keyboard.add(
        InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_previous_photo")
    )
    keyboard.add(
        InlineKeyboardButton(text="Выбрать", callback_data=f"photo_link_{version}")
    )
